...
export default function Conteudo() {
  ...
  const handleAcao = async (acao: string, id: number) => {
    switch (acao) {
      case "eliminar":
        await fetch(
          `${process.env.NEXT_PUBLIC_CLIENT_URL}/api/universidade/gerir/${id}`,
          {
            method: "DELETE",
          },
        );
        ...
        break;
        case "editar":
          router.push(`/gestao/universidade/${id}`)
          setLoading(true)
        break;
      default:
        break;
  ...
  const fetchData = async () => {
    ...
    const res = await fetch(
      ...
      }/api/universidade/gerir?${params.toString()}`,
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <Link
          href={`/gestao/universidade/registar`}
